...
def get_model_fh(data, model):
    if data == 'Digits':
        ...
        if model in ['preresnet20']:  # From heteroFL
            ...
            import timm
            ModelClass = timm.create_model('vit_tiny_patch16_224_slim', pretrained=True)
        elif model in ['vit_tiny_timm_un']:
            # from nets.HeteFL import timm
            import timm
            # ModelClass = timm.create_model('vit_tiny_patch16_224_slim', pretrained=False, class_token=False, global_pool='avg')
            ModelClass = timm.create_model('vit_tiny_patch16_224_slim', pretrained=False)
            # pretrained_model = timm.create_model('vit_tiny_patch16_224')
            # from timm.models.vision_transformer_slim import VisionTransformer
            # ModelClass = VisionTransformer()
            # ModelClass.load_state_dict(pretrained_model.state_dict(), strict = False)
        elif model in ['vit_small_timm']:
            # from nets.HeteFL import timm
            import timm
            pretrained_model = timm.create_model('vit_small_patch16_224')
            pass
        elif model in ['vit_base_timm']:
            # from nets.HeteFL import timm
            import timm
            pretrained_model = timm.create_model('vit_base_patch16_224')
            pass
...
if __name__ == '__main__':
    ...
    exp_folder = f'hfl_{args.data}_iclr'
    ...
    if args.enable_resize == False:
        ...
        if args.data == 'Cifar10':
            ...
            ModelClass.head = SlimmableLinear(ModelClass.head.weight.shape[1], 10)
            ModelClass.num_classes = 10
            ModelClass.slimmabe_ratios=fed.train_slim_ratios
        elif args.data == 'Cifar100':
            # ModelClass.width_scale=args.width_scale
            ModelClass.head = SlimmableLinear(ModelClass.head.weight.shape[1], 100)
