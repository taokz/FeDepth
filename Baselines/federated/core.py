...
class _Federation:
    ...
    def add_argument(cls, parser: argparse.ArgumentParser):
        ...
        parser.add_argument('--sel_user', type=int, default=3, choices=list(range(-3,4,1)),  help='if it is 0, only the smallest ones will be chosen.')
    ...
    def render_run_name(cls, args):
        if args.sel_user != 3:
            run_name = f'__sel_{args.sel_user}__pd_nuser_{args.pd_nuser}'
        else:
        ...
        if args.lr != 1e-2: run_name += f'__lr_{args.lr}'
        if args.no_mask_loss == True: run_name += f'__no_mask'
    ...
    def __init__(self, data, args):
        ...
        #self.client_sampler = UserSampler([i for i in range(self.client_num) if (int(i*4/self.client_num) > self.args.rm_user)], pr_nuser, mode='uni')
        if self.args.sel_user >= 0:
            self.client_sampler = UserSampler([i for i in range(self.client_num) if (int(i*4/self.client_num) <= self.args.sel_user)], pr_nuser, mode='uni')
        else:
            self.client_sampler = UserSampler([i for i in range(self.client_num) if (int(i*4/self.client_num) > abs(self.args.sel_user)-1)], pr_nuser, mode='uni')
...
class HeteFederation(_Federation):
    ...
    def add_argument(cls, parser: argparse.ArgumentParser):
        ...
        parser.add_argument('--slim_ratios', type=str, default='6-3-2-1',
                            help='define the slim_ratio for groups, for example, 6-3-2-1 [default]'
                                 ' means x1/6 net for the 1st group, and x1/3 for the 2nd')
    ...
    def render_run_name(cls, args):
        ...
        if args.slim_ratios != '6-3-2-1': run_name += f'__{args.slim_ratios}'
        return run_name
...
class SplitFederation(HeteFederation):
    ...
    def add_argument(cls, parser: argparse.ArgumentParser):
        ...
        parser.add_argument('--atom_slim_ratio', type=float, default=0.165,
    ...
    def render_run_name(cls, args):
        ...
        if args.atom_slim_ratio != 0.165: run_name += f"__asr{args.atom_slim_ratio}"
        return run_name
