...
class ModelAccumulator(object):
    ...
    def add(self, model_idx, model, weight):
        ...
        with torch.no_grad():
            for key in self._accum_state_dict:
                #print(len(self.local_state_dict)) always 0
                #print(key in self.local_state_dict[model_idx]) here is an error.
        ...
        # print(model_idx, "'s data size weight is: ", weight)
    ...
    def update_server_and_reset(self):
        ...
        self.check_full_accum()
        #print(self._weight_sum)
        #print("weight sum is ", self._weight_sum)
    ...
    def add(self, model_idx, model, weight, max_slim_ratio=1.0, slim_bias_idx=0,
        ...
        with torch.no_grad():
            ...
            for slim_bias_idx in slim_bias_idxs:
                ...
                # version 1, train all models
                model.switch_slim_mode(max_slim_ratio, slim_bias_idx=slim_bias_idx, out_slim_bias_idx=out_slim_bias_idx)
                # version 2, train without 1/6 width models
                #if int(model_idx * 4 / 100) > 0:
                    #model.switch_slim_mode(max_slim_ratio, slim_bias_idx=slim_bias_idx, out_slim_bias_idx=out_slim_bias_idx)
                #else:
                    #self.n_small += 1
                    # print('Client {} will not be accumulated because its max_slim_ratio is {}'.format(model_idx, max_slim_ratio))
                    #break
        ...
        with torch.no_grad():
            ...
            for k in self.server_state_dict:
                if 'num_batches_tracked' in k:
                    ...
                    #self._accum_state_dict[k].data[weight_nz_mask] *= 1. / (self._weight_sum[k][weight_nz_mask]*(10-self.n_small)/10)  # updated
