...
from .utils.slimmable_models import BaseModule, SlimmableMixin
from .utils.slimmable_ops import SlimmableConv2d, SlimmableBatchNorm2d, SlimmableLinear, SlimmableLayerNorm
...
from .layers import PatchEmbed_Slim, Mlp, DropPath, trunc_normal_, lecun_normal_
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if fix_in:
            self.norm1 = norm_layer(dim, non_slimmable=True)
        ...
        if fix_out:
            self.norm2 = norm_layer(dim, non_slimmable=fix_out)
...
class VisionTransformer(BaseModule, SlimmableMixin):
    """ Vision Transformer
    ...
    def __init__(
            ...
            drop_path_rate=0.,
            weight_init='',
            embed_layer=PatchEmbed_Slim,
            norm_layer=None,
            act_layer=None,
            block_fn=Block,
            width_scale=1., share_affine=False, slimmabe_ratios=None
    ):
        """
        Args:
            ...
            width_scale: slimmable ratio
        """
        super().__init__()
        # Modified
        self._set_slimmabe_ratios(slimmabe_ratios)
        if width_scale != 1.:
            embed_dim = embed_dim * width_scale
        norm_layer = lambda n_ch, **kwargs: SlimmableLayerNorm(n_ch, **kwargs)
        ...
        # self.pos_embed = nn.Parameter(torch.randn(1, embed_len, int(self.patch_embed.slim_scale * embed_dim)) * .02)
        self.pos_embed = nn.Parameter(torch.randn(1, embed_len, int(slimmabe_ratios * embed_dim)) * .02)
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                fix_out=False if (not fix_out) or (i < 12 - 1) else fix_out,
                fix_in=False if (not fix_in) or (i > 0) else fix_in
            )
        ...
        self.head = SlimmableLinear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def _pos_embed(self, x):
        if self.no_embed_class:
            ...
            print('000', x.shape)
            print('111', self.pos_embed.shape)
            if self.cls_token is not None:
                x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
            x = x + self.pos_embed
...
@register_model
def vit_tiny_patch16_224_slim(pretrained=False, slimmabe_ratios=None, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, slimmabe_ratios=slimmabe_ratios, **kwargs)
