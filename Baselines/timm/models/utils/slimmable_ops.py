...
# Modified
class SlimmableLayerNorm(LayerNorm, SlimmableOpMixin):
    """
    LayerNorm shared by all sub-networks in slimmable network.
    It is our implementation
    """
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True,non_slimmable=False):
        super(SlimmableLayerNorm, self).__init__(normalized_shape, elementwise_affine=elementwise_affine, eps=eps)
        self.max_num_features = normalized_shape
    ...
    def slim_ratio(self, r):
        ...
        #     raise RuntimeError(f"Try to track state when slim_ratio < 1 is {r}")
        if r < 0:
            raise RuntimeError(f"slim_ratio < 0 is not supported")
    ...
    def _forward_with_partial_weight(self, input, slim_bias_idx, out_slim_bias_idx=None):
        ...
        # ----- copy from parent implementation ----                                    
        return F.layer_norm(
            input, (self.num_features,), weight, bias, self.eps)
