...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride, norm_layer, conv_layer, scaler, option='A'):
...
class ResNet(ScalableModule):
    ...
    def __init__(self, hidden_size, block, num_blocks, num_classes=10, bn_type='bn',
        ...
        self.first = nn.Conv2d(3, hidden_size[0], kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.layer3 = self._make_layer(block, hidden_size[2], num_blocks[2], stride=2,
                                       norm_layer=norm_layer, conv_layer=conv_layer)
        self.n3 = norm_layer(hidden_size[2] * block.expansion)
        ...
        self.link2 = conv_layer(hidden_size[1], hidden_size[2], kernel_size=1, stride=int(hidden_size[2]/hidden_size[1]), bias=False)
    ...
    def forward(self, x, return_pre_clf_fea=False):
        ...
        if self.comp_flag == 1:
            if self.alter_flag == 1:
                ...
                out = self.layer1(out)
                out_temp = self.link1(out)
                out = self.link2(self.layer2(out)) + out_temp
            else:
                out = self.layer1(out)
                out = self.layer2(out)
                out_temp = self.link2(out)
                out = self.layer3(out) + out_temp
            ...
            out = self.layer1(out)
            out_temp1 = self.link1(out)
            out = self.layer2(out)
            out_temp2 = self.link2(out)
            out = self.layer3(out) + out_temp1 + out_temp2


        out = F.relu(self.n3(out))
