...
class ResNet(ScalableModule):
    ...
    def __init__(self, hidden_size, block, num_blocks, num_classes=10, bn_type='bn',
        ...
        self.conv1 = nn.Conv2d(3, hidden_size[0], kernel_size=3, stride=1, padding=1)
        self.bn1 = norm_layer(hidden_size[0]) 
    ...
    def forward(self, x, return_pre_clf_fea=False):
        out = self.scaler(self.bn1(self.conv1(x)))
...
# Instantiations
def resnet20(**kwargs):
    model = ResNet(hidden_size, Block, [3, 3, 3], **kwargs)
    model.apply(init_param)
    return model

def resnet32(**kwargs):
    model = ResNet(hidden_size, Block, [5, 5, 5], **kwargs)
    model.apply(init_param)
    return model

def resnet38(**kwargs):
    hidden_size = [32, 64, 112]
    model = ResNet(hidden_size, Block, [3, 3, 3], **kwargs)
    model.apply(init_param)
    return model
...
if __name__ == '__main__':
    ...
    model = resnet38(track_running_stats=False)
