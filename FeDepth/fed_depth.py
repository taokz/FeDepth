...
from federated.fedavg_rweight import _Federation as Federation
    ...
    if (data == 'Cifar10') or (data == 'Cifar100') or (data == 'EMNIST'):
        if sufficient > 0:
            if model in ['preresnet20']:  # From heteroFL
                ...
                import timm
                ModelClass = timm.create_model('vit_tiny_patch16_224', pretrained=True)
            ...
            if model in ['preresnet20']:  # From heteroFL
                from models.preresnet_res_light_ins import resnet20
                #from federated.learning_light_insufficient import train, test
                ModelClass = resnet20
            else:
                raise ValueError(f"Invalid model: {model}")
    else:
...
if __name__ == '__main__':
    ...
    # mutual train 
    # parser.add_argument('--mutual_train', type=str2bool, default=False, help='enable mutual learning.')
    ...
    if (args.model == 'preresnet20'):
        if (args.sufficient == 1):
            ...
            from federated.learning_light_sufficient import train, test
    else: # vit
        args.enable_resize = True
        if (args.sufficient == 1):
            from federated.learning_vit import train, test
    ...
    exp_folder = f'fedpth_{args.data}'
