...
def train(model, data_loader, optimizer, loss_fun, device, client_idx, num_clients,
    ...
    if adversary is None:
            ...
            for it in range(5):
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        if ("linear4" in name) or ("first" in name)  or ("first_norm" in name):
                        ...
                        if ("linear8" in name):
                            param.requires_grad = True
                        elif ("layer7" in name) or ("layer8" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    else:
        ...
        if int(client_idx * 4 / num_clients) == 1: 
            model.comp_flag = 2
            for it in range(6):
                data_iterator = iter(data_loader)
                model.alter_flag = it + 1
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        ...
                        if ("linear9" in name):
                            ...
                            param.requires_grad = False
                # print('0000000', client_idx, model.comp_flag, model.alter_flag)
                for step in tqdm_iters:
                # for data, target in tqdm(data_loader, file=sys.stdout):
                    try:
                        data, target = next(data_iterator)
