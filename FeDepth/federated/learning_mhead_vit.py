    ...
    if adversary is None:
        ...
        if int(client_idx * 4 / num_clients) == 0: 
            ...
            for it in range(6):
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        ...
                        elif ("blocks.0" in name) or ("blocks.1" in name) or ("aux_norm1" in name) or ("aux_head1" in name):
                            param.requires_grad = True
                        else:
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name):
                            param.requires_grad = True
                        elif ("blocks.2" in name) or ("blocks.3" in name) or ("aux_norm2" in name) or ("aux_head2" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name):
                            param.requires_grad = True
                        elif ("blocks.4" in name) or ("blocks.5" in name) or ("aux_norm3" in name) or ("aux_head3" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name):
                            param.requires_grad = True
                        elif ("blocks.6" in name) or ("blocks.7" in name) or ("aux_norm4" in name) or ("aux_head4" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name):
                            param.requires_grad = True
                        elif ("blocks.8" in name) or ("blocks.9" in name) or ("aux_norm5" in name) or ("aux_head5" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    else:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head." in name):
                            param.requires_grad = True
                        elif ("blocks.10" in name) or ("blocks.11" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
            ...
            for it in range(3):
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("aux_head2" in name):
                            param.requires_grad = True
                        elif ("blocks.0" in name) or ("blocks.1" in name) or ("blocks.2" in name) or ("blocks.3" in name) or ("aux_norm2" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    elif model.alter_flag == 2:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("aux_head4" in name):
                            param.requires_grad = True
                        elif ("blocks.4" in name) or ("blocks.5" in name) or ("blocks.6" in name) or ("blocks.7" in name) or ("aux_norm4" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    else:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head." in name):
                            param.requires_grad = True
                        elif ("blocks.8" in name) or ("blocks.9" in name) or ("blocks.10" in name) or ("blocks.11" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
            ...
            for it in range(2):
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("aux_head3" in name):
                            param.requires_grad = True
                        elif ("blocks.0" in name) or ("blocks.1" in name) or ("blocks.2" in name) or ("blocks.3" in name) or ("blocks.4" in name) or ("blocks.5" in name)  or ("aux_norm3" in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    elif model.alter_flag == 2:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head." in name):
                            param.requires_grad = True
                        elif ("blocks.6" in name) or ("blocks.7" in name) or ("blocks.8" in name) or ("blocks.9" in name) or ("blocks.10" in name) or ("blocks.11" in name)  or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
