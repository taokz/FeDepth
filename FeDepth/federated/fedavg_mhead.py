...
from federated.aggregation_mhead import ModelAccumulator
...
class _Federation:
    ...
    def render_run_name(cls, args):
        ...
        if args.percent != 1: run_name += f'__pct_{args.percent}'
    ...
    def __init__(self, data, args):
        ...
        # self.total_client_weights = sum(client_weights)
        self.client_weights = [w / sum(client_weights) for w in client_weights]
        # Updated
        # self.client_weights = [len(tl.dataset) for tl in train_loaders]
    ...
    # def download(self, running_model, client_idx, strict=False):
