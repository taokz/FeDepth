            ...
            for it in range(6):
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        ...
                        elif ("blocks.0" in name) or ("blocks.1" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head" in name):
                            param.requires_grad = True
                        elif ("blocks.2" in name) or ("blocks.3" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head" in name):
                            param.requires_grad = True
                        elif ("blocks.4" in name) or ("blocks.5" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head" in name):
                            param.requires_grad = True
                        elif ("blocks.6" in name) or ("blocks.7" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                        ...
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head" in name):
                            param.requires_grad = True
                        elif ("blocks.8" in name) or ("blocks.9" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    else:
            ...
            for it in range(3):
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head" in name):
                            param.requires_grad = True
                        elif ("blocks.0" in name) or ("blocks.1" in name) or ("blocks.2" in name) or ("blocks.3" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    elif model.alter_flag == 2:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head" in name):
                            param.requires_grad = True
                        elif ("blocks.4" in name) or ("blocks.5" in name) or ("blocks.6" in name) or ("blocks.7" in name) or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    else:
            ...
            for it in range(2):
                ...
                for name, param in model.named_parameters():
                    if model.alter_flag == 1:
                        if ("cls_token" in name) or ("pos_embed" in name) or ("patch_embed" in name) or ("head" in name):
                            param.requires_grad = True
                        elif ("blocks.0" in name) or ("blocks.1" in name) or ("blocks.2" in name) or ("blocks.3" in name) or ("blocks.4" in name) or ("blocks.5" in name)  or ("norm." in name):
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    elif model.alter_flag == 2:
