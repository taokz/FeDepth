...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            act_layer=None,
            block_fn=Block,
            comp_flag=None, alter_flag=None,
    ):
        """
        Args:
        ...
        self.aux_norm1 = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()
        self.aux_norm2 = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()
        self.aux_norm3 = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()
        self.aux_norm4 = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()
        self.aux_norm5 = norm_layer(embed_dim) if not use_fc_norm else nn.Identity()
        
        # Classifier Head
        self.fc_norm = norm_layer(embed_dim) if use_fc_norm else nn.Identity()
        self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.aux_head1 = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.aux_head2 = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.aux_head3 = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.aux_head4 = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.aux_head5 = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        x = self._pos_embed(x)
        x = self.norm_pre(x)
        # I think grad_checkpointing is not active by default, so I just modify the 'else'
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        if self.comp_flag == 1:
            if self.alter_flag == 1:
                return x if pre_logits else self.aux_head1(x)
            elif self.alter_flag == 2:
                return x if pre_logits else self.aux_head2(x)
            elif self.alter_flag == 3:
                return x if pre_logits else self.aux_head3(x)
            elif self.alter_flag == 4:
                return x if pre_logits else self.aux_head4(x)
            elif self.alter_flag == 5:
                return x if pre_logits else self.aux_head5(x)
            else:
...
@register_model
def vit_tiny_patch16_224_mhead(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
