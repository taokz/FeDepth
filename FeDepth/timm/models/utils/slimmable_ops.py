...
# Modified
class _SlimmableLayerNorm(LayerNorm, SlimmableOpMixin):
    """
    LayerNorm shared by all sub-networks in slimmable network.
    It is our implementation
    """
    def __init__(self, normalized_shape, num_features, eps=1e-5, elementwise_affine=True,non_slimmable=False):
        super(_SlimmableLayerNorm, self).__init__(normalized_shape, num_features, elementwise_affine=elementwise_affine, eps=eps)
        self.max_num_features = num_features
        self._slim_ratio = 1.0
        self.slim_bias_idx = 0
        self.out_slim_bias_idx = None
