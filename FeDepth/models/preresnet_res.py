    ...
    def forward(self, x, return_pre_clf_fea=False):
        ...
        if self.comp_flag == 1:
            if self.alter_flag == 1:
                ...
                out = self.layer1(out)
                out_temp = F.avg_pool2d(out, 2)
                out = self.layer2(out) + out_temp
            ...
            out = self.layer1(out)
            out_temp1 = F.avg_pool2d(out, 8)
            out = self.layer2(out)
            out_temp2 = F.avg_pool2d(out, 4)
            out = self.layer3(out)
            out_temp3 = F.avg_pool2d(out, 2)
            out = self.layer4(out) + out_temp1 + out_temp2 + out_temp3
            #out = self.layer4(out)
...
if __name__ == '__main__':
    from .util.profile_func import profile_model
