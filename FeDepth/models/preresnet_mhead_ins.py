...
class ResNet(ScalableModule):
    ...
    def __init__(self, hidden_size, block, num_blocks, num_classes=10, bn_type='bn',
        ...
        self.pre_linear1 = norm_layer(hidden_size[0])
        self.pre_linear2 = norm_layer(hidden_size[0])
        self.pre_linear3 = norm_layer(hidden_size[0])
        self.pre_linear4 = norm_layer(hidden_size[1])
        self.pre_linear5 = norm_layer(hidden_size[1])
        self.pre_linear6 = norm_layer(hidden_size[1])
        self.pre_linear8 = norm_layer(hidden_size[2])
        self.pre_linear9 = norm_layer(hidden_size[2])

        self.linear1 = nn.Linear(hidden_size[0] * block.expansion, num_classes)
        self.linear2 = nn.Linear(hidden_size[0] * block.expansion, num_classes)
        self.linear3 = nn.Linear(hidden_size[0] * block.expansion, num_classes)
        self.linear4 = nn.Linear(hidden_size[1] * block.expansion, num_classes)
        self.linear5 = nn.Linear(hidden_size[1] * block.expansion, num_classes)
        self.linear6 = nn.Linear(hidden_size[1] * block.expansion, num_classes)
        self.linear8 = nn.Linear(hidden_size[2] * block.expansion, num_classes)
        self.linear9 = nn.Linear(hidden_size[2] * block.expansion, num_classes)
    ...
    def forward(self, x, return_pre_clf_fea=False):
        ...
        if self.comp_flag == 1:
            if self.alter_flag == 1:
                ...
                out = self.layer4(out)
                out = self.layer5(out)
                out = self.pre_linear5(out)
                out = F.adaptive_avg_pool2d(out, 1)
                out = out.view(out.size(0), -1)
                logits = self.linear5(out)
                if return_pre_clf_fea:
                    return logits, out
                else:
                    return logits
                ...
                out = self.layer5(out)
                out = self.layer6(out)
                out = self.pre_linear6(out)
                out = F.adaptive_avg_pool2d(out, 1)
                out = out.view(out.size(0), -1)
                logits = self.linear6(out)
                if return_pre_clf_fea:
                    return logits, out
                else:
                    return logits
                ...
                out = self.layer7(out)
                out = self.layer8(out)
                out = self.pre_linear8(out)
                out = F.adaptive_avg_pool2d(out, 1)
                out = out.view(out.size(0), -1)
                logits = self.linear8(out)
                if return_pre_clf_fea:
                    return logits, out
                else:
                    return logits
                ...
                out = self.layer8(out)
                out = self.layer9(out)
                out = self.pre_linear9(out)
                out = F.adaptive_avg_pool2d(out, 1)
                out = out.view(out.size(0), -1)
                logits = self.linear9(out)
                if return_pre_clf_fea:
                    return logits, out
                else:
                    return logits
