...
def get_central_data(name: str, domains: list, enable_resize=False):
    ...
    test_sets = [CifarDataset(domain, train=False, transform=compose_transforms('test', enable_resize=enable_resize)) for domain in domains]
    return train_sets, test_sets

def get_central_data_100(name: str, domains: list, enable_resize=False):
    train_sets = [CifarDataset100(domain, train=True, transform=compose_transforms('train', enable_resize=enable_resize)) for domain in domains]
    test_sets = [CifarDataset100(domain, train=False, transform=compose_transforms('test', enable_resize=enable_resize)) for domain in domains]
    return train_sets, test_sets
...
def make_fed_data(train_sets, test_sets, batch_size, domains, shuffle_eval=False,
    ...
    """distribute training datasets to clients.
	Args:
		train_sets (list): A list of datasets for training
		test_sets (list): A list of dataset for testing
		consistent_test_class (bool): Ensure the test classes are the same traning for a client.
...
def prepare_cifar100_data(args, domains=['cifar100'], shuffle_eval=False, n_class_per_user=-1,
    ...
    train_sets, test_sets = get_central_data_100('cifar100', domains, enable_resize=enable_resize)
